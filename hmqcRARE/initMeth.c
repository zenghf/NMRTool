...
/*:=MPE=:=======================================================*/
{
  ...
  if(ParxRelsParHasValue("N15Pulse") == No)
    STB_InitRFPulse(&N15Pulse,
                     CFG_RFPulseDefaultShapename(LIB_EXCITATION),
                     1.0,  /* default duration in ms */
                     90.0);
  N15PulseRange(); 
  ...
  STB_InitExcPulseEnum("N15PulseEnum");
  ...
  */
  ...
  printf("initMeth: if(PVM_NumberOfNuclei!=2)\n");
    ...
    printf("initMeth: iSTB_InitNuclei(2);\n");
  ...
  ParxRelsMakeNonEditable("PVM_NRepetitions");
  if(ParxRelsParHasValue("dJCouple") == No)
    dJCouple = 5.6;
  if(ParxRelsParHasValue("dEvolve") == No)
    dEvolve = 0.0;
  if(ParxRelsParHasValue("dExchange") == No)
    dExchange = 60;
  if(ParxRelsParHasValue("gradSpoiler") == No)
    gradSpoiler = 0.1;
  if(ParxRelsParHasValue("hmqcAcqOrder") == No)
    hmqcAcqOrder = ON_OFF;
  if(ParxRelsParHasValue("nExchange") == No)
    nExchange = 30;
  if(ParxRelsParHasValue("nRepeat") == No)
    nRepeat = 1;
  PARX_change_dims("onOffOption",2);
