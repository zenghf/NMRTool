...
void backbone( void )
{
  ...
  STB_UpdateRFPulse("H90Pulse",
                    &H90Pulse,
                    refAttIsAvailable,
                    referenceAttenuation);
  STB_UpdateExcPulseEnum("H90PulseEnum",
                         &H90PulseEnum,
                         H90Pulse.Filename,
                         H90Pulse.Classification);
  
  STB_UpdateRFPulse("H180Pulse",
                    &H180Pulse,
                    refAttIsAvailable,
                    referenceAttenuation);
  
  STB_UpdateRfcPulseEnum("H180PulseEnum",
                         &H180PulseEnum,
                         H180Pulse.Filename,
                         H180Pulse.Classification);
/**********Jiadi Xu **Update Noe ref*******/
 STB_UpdateNoeModule(PVM_Nucleus2);
  
  
  /* 
  STB_UpdateRFPulse("N15Pulse",
                    &N15Pulse,
                    refAttIsAvailable,
                    referenceAttenuation);
  */
  ...
  STB_UpdateExcPulseEnum("N15PulseEnum",
                         &N15PulseEnum,
                         N15Pulse.Filename,
                         N15Pulse.Classification);
  ...
  if(PVM_NumberOfNuclei!=2)
  {
    ...
    if(PVM_NumberOfNuclei < 2)
    {
      ...
      UT_ReportError("system configuration does not support 2nd RF channel!");
    }
  }
...
/******************************Added by HZeng ***************************/
void H90PulseEnumRelation(void)
{
  YesNo status;
  status = STB_UpdateExcPulseName("H90PulseEnum",
                                  &H90PulseEnum,
                                  H90Pulse.Filename,
                                  &H90Pulse.Classification);
  backbone();
...
void H90PulseRelation(void)
{
  UT_SetRequest("H90Pulse"); 
  backbone();
}

void H180PulseEnumRelation(void)
{
  YesNo status;
  status = STB_UpdateRfcPulseName("H180PulseEnum",
                                  &H180PulseEnum,
                                  H180Pulse.Filename,
                                  &H180Pulse.Classification);
  backbone();
...
void H180PulseRelation(void)
{
  UT_SetRequest("H180Pulse"); 
  backbone();
}

void N15PulseEnumRelation(void)
{
  YesNo status;

  DB_MSG(("-->N15PulsesEnumRelation\n"));
  
  /* set the name and clasification of ExcPulse: */
  status = STB_UpdateExcPulseName("N15PulseEnum",
                                  &N15PulseEnum,
                                  N15Pulse.Filename,
                                  &N15Pulse.Classification);
  ...
  DB_MSG(("<--N15PulseEnumRelation\n"));
}


void N15PulseRelation(void)
{
  DB_MSG(("-->N15PulseRelation\n"));
...
void N15PulseRange(void)
{
  DB_MSG(("-->N15PulseRange\n"));
  
  /* allowed clasification */
  
  switch(N15Pulse.Classification)
  {
    default:
      N15Pulse.Classification = LIB_EXCITATION;
      break;
    case LIB_EXCITATION:
    case PVM_EXCITATION:
    case USER_PULSE:
  ...
  /* N15Pulse.FlipAngle = MIN_OF(90.0,ExcPulse.FlipAngle); */
  
  
  /* general verifiation of all pulse atributes  */
  
  STB_CheckRFPulse(&N15Pulse);
...
void HMQCRels(void) {
  dJCouple = MAX_OF(dJCouple, 0.0);
  dEvolve = MAX_OF(dEvolve, 0.0);
  dExchange = MAX_OF(dExchange, 0.0);
  gradSpoiler = MAX_OF(gradSpoiler, 0.0);
  // hmqcAcqOrder = ON_OFF;
  nExchange = MAX_OF(nExchange, 1);
  nRepeat = MAX_OF(nRepeat, 1);
  PVM_NRepetitions= MAX_OF(nRepeat*2, 2);
  // PARX_change_dims("onOffOption",2);
  if (hmqcAcqOrder == ON_OFF) {
    onOffOption[0] = 1;
    onOffOption[1] = 0;
  }
  else if (hmqcAcqOrder == OFF_ON) {
    onOffOption[0] = 0;
    onOffOption[1] = 1;
  }
