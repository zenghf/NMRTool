...
void initFPRel(void) {
  ...
  ParxRelsMakeNonEditable("CS_MSG, FP_NAcq, FP_delayInv, FP_flipAngleList, FP_TEList");
  FP_NAcq = 1;
  //FP_delayInv = (d4*2) - d3 - d8 - d9 - (p2 * 0.5) - (p0 * 0.5);
  FP_delayInv = 0.1;
  PARX_change_dims("FP_flipAngleList", FP_NAcq);
  PARX_change_dims("FP_TEList", FP_NAcq);
  FP_flipAngleList[0] = 90.0;
  FP_TEList[0] = PVM_EchoTime;

  printf("<---- initFPRel\n");
}

/* calculate varTEList, flipPowerList */
void calcFPRel(void) {
  printf("----> calcFPRel");
  PARX_change_dims("varTEList", FP_NAcq);
  PARX_change_dims("flipPowerList", FP_NAcq);
  int i = 0;
  for (i = 0; i < FP_NAcq; i++) {
    varTEList[i] = FP_TEList[i] - PVM_EchoTime;
...
void readFPRel(void)
{
  ...
  if (fd != NULL) {
    ...
    // read FP_flipAngleList, FP_TEList
    i = 0;
