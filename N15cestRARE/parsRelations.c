  ...
  {  
    PVM_MinRepetitionTime =
      nSlices * (
	       ...
	       PVM_ExSlicePulseLength / 2.0 +
	       PVM_EchoTime*NEchoes +
	       tail +NPulse*(CESTPulse.Length+CEST_interdelay)+/**JXU**/
	       PVM_InterGradientWaitTime /* min d0 */
	       );
...
NPulse=MAX_OF(1,NPulse);
CEST_interdelay=MAX_OF(0,CEST_interdelay);
/*************Define pulses********/

/*jiadi xu Pulse90*/
  if(ParxRelsParHasValue("CEST_attref") == No)
   {  
    STB_InitRFPulse(&CESTPulse,
		      "gauss.inv",
		     5.0,/* default duration in ms */
		     180.0);

    STB_UpdateInvPulseEnum("CESTPulseEnum",
			 &CESTPulseEnum,
			 CESTPulse.Filename,
			 CESTPulse.Classification);

   }
/*jiadi xu */

 
STB_SpecRFPulseUpdate("CESTPulse",
		    &CESTPulse,
                    2,
                    "",
	             Yes,
  ...
  STB_UpdateInvPulseName("CESTPulseEnum",
				  &CESTPulseEnum,
				  CESTPulse.Filename,
				  &CESTPulse.Classification);
  ...
  /* Tell the request handling system that the parameter
     RefPulse has been edited */
  UT_SetRequest("CESTPulse");


  /* general verifiation of all pulse atributes  */

  STB_CheckRFPulse(&CESTPulse);

  CEST_uT=db2ut(CESTPulse.Attenuation, CEST_attref);

ParxRelsMakeNonEditable("CEST_uT");
/***************pulse update************/

/**************************/
ParxRelsShowInEditor("MTOffsetMode,MT_Offsets");
ParxRelsShowInEditor("Initial_MT_Offset,Final_MT_Offset,Offset_Step_Hz");
ParxRelsShowInEditor("Number_Offsets");
ParxRelsMakeNonEditable("MT_Offsets");
...
if (MTOffsetMode== Enter_Manually)
  {
   ...
   ParxRelsMakeEditable("MT_Offsets");
...
/************************************************************************/
      ...
      PARX_change_dims("MT_Offsets",Number_Offsets);
  ...
  switch(MTOffsetMode)
  {
   case Sequential_Offset:
    ...
    {
     MT_Offsets[i] = Initial_MT_Offset+i*Offset_Step_Hz;
    }
    ...
    Offset_Step_Hz=0;
     /***********Offset frequency list range check**********/
   	 Initial_MT_Offset= MAX_OF(Initial_MT_Offset,-500000);
    	 Initial_MT_Offset = MIN_OF(Initial_MT_Offset,500000);
         Final_MT_Offset= MAX_OF(Final_MT_Offset,-500000);
   	 Final_MT_Offset = MIN_OF(Final_MT_Offset,500000);
    ...
    for ( i=0; i < Number_Offsets; i++)
    {
     ...
     MT_Offsets[inter_i] = Initial_MT_Offset+i*Offset_Step_Hz;
    }
   ...
   case From_File:
    ...
    PARX_change_dims("MT_Offsets",2000);
     i=0;
     MT_Offsets[0]=0;
    if ((fd=fopen(MTListFile,"r"))!=NULL)
      {
       while(fgets(line,256,fd)!=NULL)
            {
         MT_Offsets[i]=round(atof(line)*SFO1); 
         i=i+1;    
             }
 ...
 PVM_NRepetitions= MAX_OF(Number_Offsets,1);
}
/****************************************************************/
/*		E N D   O F   F I L E				*/
/****************************************************************/

double db2ut(double db, double refatt)
{

double H_gym,satHz;
double satT;
satHz=250*pow(10,(refatt-db)/20);

H_gym=26.7520*10/(2*3.1415);
satT=satHz/H_gym;
return (satT);
}
