...
/*JXU cs init and relation check*/
...
void readConfig(void){
  ...
  if (fd != NULL) {
    ...
    fgets(line, 1024, fd);
    // read CS_EncMatrix
    if (dim == 2) {sscanf(line, "%s %d", str1, &n1); printf("%d\n", n1);}
    if (dim == 3) {sscanf(line, "%s %d %d", str1, &n1, &n2); printf("%d %d\n", n1, n2);}
    ...
    if (dim == 2) {sscanf(line, "%s %d", str1, &n3);}
    if (dim == 3) {sscanf(line, "%s %d %d", str1, &n3, &n4);}
    ...
    if (dim ==3) {CS_ACT_EncMatrix[2] = n4;}
  ...
  if (flagReadFile == 1) {
    ...
    if (fd != NULL) {
      ...
      while( fgets(line, 1024, fd) != NULL && i < totalEncSize) {
        if (dim == 2) {sscanf(line, "%d", &n1);}
        if (dim == 3) {sscanf(line, "%d %d", &n1, &n2);}
        CS_EncSteps_1[i] = n1;
