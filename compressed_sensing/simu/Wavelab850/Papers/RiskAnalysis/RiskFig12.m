...
x_length = 1024;
sigchoice = zeros(1,10); sigchoice(4) = 1;sigchoice(8) = 1;
% fig_hand3 = figure;
...
disp('***  This produces Figure 12 of the Paper ***');
...
b = 1:10; b = b(sigchoice == 1);
  for i = 1:nplot,
   ...
   [x_symm_risk_low,u_symm] = ...
      CalcThreshValueRisk(x_symm,x_length,0.02);
   [x_symm_risk_high,u_symm] = ...
      CalcThreshValueRisk(x_symm,x_length,0.1);
...
line([x0 x0],[y0 y0],[zmin zmax],'LineStyle','--');
      zlabel('log10(Risk)');
titlestr = ...
['#' num2str(b(i)) '  ' signal_name ':  sigma = 0.02, Basis : Symmlet 8'];
      title(titlestr);
      hold off;
...
line([x0 x0],[y0 y0],[zmin zmax],'LineStyle','--');
      zlabel('log10(Risk)');
titlestr = ...
['#' num2str(b(i)) '  ' signal_name ':  sigma = 0.1, Basis : Symmlet 8'];
      title(titlestr);
      hold off;
end    
...
disp('*        Type of Thresholding:  Hard                          *');
disp('***************************************************************');
