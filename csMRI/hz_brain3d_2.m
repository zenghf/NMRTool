...
D2 = size(img0,2);
D3 = size(img0,3);
I1 = D1; % 2^nextpow2(max(D1,D2));
I2 = D2; % 2^nextpow2(max(D1,D2));
...
XFM = 1; %Wavelet('Haar',4,4);	% Wavelet
...
for k = 1:NFid
    ...
    for n=1:1
        %param.xfmWeight = 0.00 * (1^n);
        %param.TVWeight =TVWeight * 0.71^n;     % TV penalty 
        %param.xfmWeight = xfmWeight * 0.83^n;  % L1 wavelet penalty
 ...
 toc
view3dgui(abs(img_recon));
view3dgui(abs(img0));
%view3dgui(abs(img_recon-img0));
view3dgui(abs(abs(img_recon)-abs(img0)));
mean(abs(img0(:) - img_recon(:)))
max(abs(img0(:) - img_recon(:)))