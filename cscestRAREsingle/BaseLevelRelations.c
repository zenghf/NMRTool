...
void SetBaseLevelParam( void )
{
  ...
  {
    ATB_SetPulprog("cscestRAREsingle.4ch");
  }
...
void SetFrequencyParameters( void )
{
  ...
  /*Jiadi XU add*/
 if (PVM_MagTransOnOff == On)
{
    
    /********Defining the frequency list  elements ******/
	
         ACQ_O3_list_size=Number_Offset_Experiments;

         ParxRelsParRelations("ACQ_O3_list_size",Yes);
 

         PARX_change_dims("ACQ_O3_list", PVM_NRepetitions);
   

         for( i=0; i <  PVM_NRepetitions; i++ )
           {
           ACQ_O3_list[i]=MT_Offsets[i];
...
void SetGradientParameters( void )
{
  ...
  switch(spatDim)
  {
    case 3:
      ...
      ParxRelsCopyPar("ACQ_spatial_phase_2","CS_EncValues_2");
      /* no break */
    case 2:
      ...
      ParxRelsCopyPar("ACQ_spatial_phase_1","CS_EncValues_1");
      /* no break */
    default:
...
void SetPpgParameters( void )
{
  ...
  ATB_SetPulprog("cscestRAREsingle.ppg");
  if(PVM_SelIrOnOff)
  {
